package com.example.Fabulous.Utils.DataProvider;
...
public class VideosDataProvider extends AbstractExpandableDataProvider {
    ...
    public VideosDataProvider(ProfileDetails.Training training) {

        mData = new LinkedList<>();

        for (int i = 0; i < training.sections.size(); i++) {

            final long groupId = i;
            final String groupText = training.sections.get(i).sectionTitle;
            final ConcreteGroupData group = new ConcreteGroupData(groupId, groupText);
            final List<ChildData> children = new ArrayList<>();

            for (int j = 0; j < training.sections.get(i).courses.size(); j++) {
                final long childId = group.generateNewChildId();
                final String childText = training.sections.get(i).courses.get(j).title;
